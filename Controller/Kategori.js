...
router.get("/", async function(req, res, next) {
    const { timestamp = false, cari = "" } = req.query;
    let sql,
        result = [];
    if (timestamp) {
        sql = `select * from tblkategori where idtoko=$1 and nama_kategori ilike '%${cari}%'  and (created_at > $2 or updated_at > $2) order by nama_kategori`;
        result = await koneksi.query(sql, [req.context.idtoko, timestamp]);
    } else {
        sql = `select * from tblkategori where idtoko=$1 and nama_kategori ilike '%${cari}%' order by nama_kategori`;
...
router.get("/:id", async function(req, res, next) {
    ...
    let result = await koneksi.oneOrNone(
        "select * from tblkategori where idkategori = $1 and idtoko=$2", [id, req.context.idtoko]
    );
