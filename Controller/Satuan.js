...
router.get("/", async function(req, res, next) {
    ...
    if (timestamp) {
        result = await koneksi.query(
            `select * from tblsatuan where  nama_satuan ilike '%${cari}%' and (created_at > $2 or updated_at > $2) and idtoko=$1`, [req.context.idtoko, timestamp]
        );
    } else {
        result = await koneksi.query(
            `select * from tblsatuan where nama_satuan ilike '%${cari}%' and idtoko=$1`, [req.context.idtoko]
        );
    }
...
router.get("/:id", async function(req, res, next) {
    ...
    let result = await koneksi.oneOrNone(
        "select * from tblsatuan where idsatuan = $1", [id]
    );
