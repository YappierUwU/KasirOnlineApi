...
router.post("/barang", handlerInput, async function (req, res, next) {
  try {
    let { idsatuan } = await koneksi.oneOrNone(
      "insert into tblsatuan (nama_satuan,idtoko) values ($1,$2) returning idsatuan",
      [req.body.nama_satuan, req.context.idtoko]
    );
    let { idkategori } = await koneksi.oneOrNone(
      "insert into tblkategori (nama_kategori,idtoko) values ($1,$2) returning idkategori",
      [req.body.nama_kategori, req.context.idtoko]
    );
